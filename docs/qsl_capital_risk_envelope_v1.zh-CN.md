...
| **量化系统** | 按权益落入资金信封；算 `capital_scale`、产品杠杆帽、回撤刹车；裁切策略目标；组合相关组上限；fail-closed | 因净值上涨静默升到更激进档；绕过 RiskEngine |
...
进阶数字（1.50× MDD、0.75 size、`capital_scale`）折叠在「详情」，避免把人推入双口径迷宫。
...
策略目标权重
  ...
  × 插件 scalar（≤1）
  × capital_scale（本信封；档内封顶，只可降不可静默升）
      → 组合预算裁切（可选）
...
| **资金信封 `capital_scale`** | 按权益再缩放，并限制杠杆产品与回撤刹车 | **系统按表计算**；人只在跨档策略变化时点确认 |
...
3. 相关组（如 TQQQ+SOXL）：组合预算之和不得突破信封允许的总风险袖。  
...
| D1 | QPK 纯函数 `equity → envelope` + 单测 | 与晋级 sizing 组合的单元证明 |
| D2 | 注入对账权益到账户新风险门；超限只禁新风险 | 无自动平仓、无复位 |
| D3 | 多账户汇总视图共用信封 | 仍无共账户 allocator |

## 7. 非目标

- 凯利公式主仓位、AI 调档、Composer 自动写生产政策；  
- 用自评净资产自动提高杠杆；  
