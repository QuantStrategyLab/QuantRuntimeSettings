...
**当前状态**：部分完成（QPK `strategy_lifecycle/backtest_orchestrator.py` 已落地；CN/HK/US/Crypto 各有 walk_forward pilot；**3c 全量推广未完成**）
...
**当前状态**：部分完成（四策略仓 `drift-check.yml` + entrypoint `PerformanceMonitor.record()` 已接线；live 执行结果回灌仍待平台层全面收口）
...
| 项目 | 状态 | 说明 |
|---|---|---|
| 任务 3c：BacktestOrchestrator 推广 | 未完成 | walk_forward pilot 已有 CN/HK/US/Crypto，尚未替换全部 ad-hoc 回测脚本 |
| 任务 5：DriftDetector 全链路 | 部分完成 | CI drift-check 已接线；live 执行结果 → PerformanceMonitor 回灌仍待平台层全面收口 |
| 任务 12：模型分层路由 | 部分完成 | AIAuditBridge 已路由；VPS Codex / CI gateway 尚未统一走 `model_router` |
| 任务 1–11 / Binance / QPK 自动化 / VPS 验真 | 已完成 | 见上文各任务「当前状态」与已完成改动表 |
