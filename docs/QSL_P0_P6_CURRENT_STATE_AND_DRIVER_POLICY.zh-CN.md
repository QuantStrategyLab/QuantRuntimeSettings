...
为避免把“可复用流程”误写成“共用策略”，`UsEquitySnapshotPipelines` 的多策略研究 Driver 目录只登记每条路线自己的 P1 输入契约、P2 配置摘要、P3 入口和迁移状态：TQQQ 是已接日更研究的样板，SOXL/SOXX 已有新的 P2 v2 与 P1 数据身份契约，但仍是待迁移路线。这个目录不调用任何路线，也不让 SOXL 继承 TQQQ 的数据、参数、证据或权限。
...
| 多策略 P1–P3 Driver 目录 | 已接线（描述层） | TQQQ 标为已接日更研究；SOXL/SOXX 的新 P2 v2 与 P1 数据身份契约标为 `MIGRATION_REQUIRED`，尚缺 P1 publisher、隔离 P3 verifier 和 scheduler。旧固定截止日/九资产 Gateway 研究只保留为历史上下文，不能接入新路线。目录不调度、不获取数据、不调用插件或策略，且不授予 P4–P6。 |
...
- `2026-08-20`：`UsEquityStrategies` 合入 SOXL/SOXX core-only P2 v2 public research adapter（PR #335）；`UsEquitySnapshotPipelines` 随后合入冻结候选（PR #336）和三资产 P1 数据身份/manifest 契约（PR #337），相关 CI 均通过。它们不读取真实行情、凭证或云端对象，不运行 P3，也不调度或启用 paper、shadow、live。后续 SOXL P3 必须用独立、可复现的 source/runtime，不能把其依赖升级施加到 TQQQ 既有日更链。
