...
| AI 持续观察 | 已接线（issue-only） | AIAuditBridge 只在两次可比较观察后创建/更新 Issue；不会执行实验或修改系统。 |
| `qsl.research_task.v1` 与控制台队列 | consumer 已实现；producer/config 待合并接线 | 独立任务来源索引会复核 SHA、revision、摘要和 no-order authority；空队列不是故障，也不能由 Issue 推断任务。 |
...
因此目前唯一的自动闭环是“观察 → 记录/告警 → 受限研究任务（接线后）”，不是“观察 → 自动调参/自动改代码/自动交易”。任何文档若与本表相冲突，应标记为历史或草稿，而不是扩大当前系统能力。
