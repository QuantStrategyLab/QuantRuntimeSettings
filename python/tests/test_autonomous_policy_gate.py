...
class AutonomousPolicyGateTest(unittest.TestCase):
    ...
    def test_contract_schemas_are_closed_and_do_not_claim_runtime_authority(self):
        ...
        forward_observation_schema = json.loads(
            (ROOT.parent / "schemas" / "qsl-forward-observation-risk-control.v1.schema.json").read_text()
        )
        ...
        self.assertEqual(forward_observation_schema["$id"], "qsl.forward_observation_risk_control.v1")
        self.assertFalse(forward_observation_schema["additionalProperties"])
        self.assertIn("does not authorize", forward_observation_schema["description"])
