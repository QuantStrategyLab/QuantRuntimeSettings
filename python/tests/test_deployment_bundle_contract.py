...
class DeploymentBundleContractTest(unittest.TestCase):
    ...
    def test_schema_is_closed_and_declares_utc_rfc3339_timestamp(self):
        schema = json.loads((ROOT.parent / "schemas" / "qsl-deployment-bundle.v1.schema.json").read_text())
        self.assertEqual(schema["$id"], "qsl.deployment_bundle.v1")
        self.assertFalse(schema["additionalProperties"])
        self.assertEqual(schema["properties"]["created_at"]["format"], "date-time")
        self.assertEqual(schema["properties"]["created_at"]["pattern"], "^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}Z$")
        self.assertFalse(schema["properties"]["dependencies"]["additionalProperties"])
        self.assertEqual(set(schema["properties"]["dependencies"]["required"]), {"qpk", "strategy", "pipeline", "platform"})
    ...
    def test_strategy_revision_must_equal_dependency_before_hash(self):
        bundle = self._bundle()
        bundle["strategy"]["revision"] = self._revision("0")
        with self.assertRaisesRegex(deployment_bundle_contract.BundleValidationError, "strategy.revision"):
            deployment_bundle_contract.validate_bundle(bundle)
    ...
    def test_invalid_calendar_timestamps_fail_closed(self):
        for timestamp in (
            ...
            with self.subTest(timestamp=timestamp):
                ...
                with self.assertRaisesRegex(deployment_bundle_contract.BundleValidationError, "timestamp"):
                    deployment_bundle_contract.validate_bundle(bundle)
